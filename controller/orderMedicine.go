...
func GetCompany(c *gin.Context) {
	var MedicineCompany entity.MedicineCompany
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM medicine_companies WHERE id = ?", id).Scan(&MedicineCompany).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListCompany(c *gin.Context) {
	var MedicineCompany []entity.MedicineCompany
	if err := entity.DB().Raw("SELECT * FROM medicine_companies").Scan(&MedicineCompany).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func DeleteCompany(c *gin.Context) {
	id := c.Param("id")
	if tx := entity.DB().Exec("DELETE FROM medicine_companies WHERE id = ?", id); tx.RowsAffected == 0 {
...
func GetOrder(c *gin.Context) {
	var order entity.MedicineOrder
	id := c.Param("id")
	if err := entity.DB().Preload("Employee").Preload("Medicine").Preload("MedicineCompany").Raw("SELECT * FROM medicine_orders WHERE id = ?", id).Find(&order).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListOrder(c *gin.Context) {
	var order []entity.MedicineOrder
	if err := entity.DB().Preload("Employee").Preload("Medicine").Preload("MedicineCompany").Raw("SELECT * FROM medicine_orders").Find(&order).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func DeleteOrder(c *gin.Context) {
	id := c.Param("id")
	if tx := entity.DB().Exec("DELETE FROM medicine_orders WHERE id = ?", id); tx.RowsAffected == 0 {
